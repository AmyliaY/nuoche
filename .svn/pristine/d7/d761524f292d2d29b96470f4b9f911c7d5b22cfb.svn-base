...
private ProxyDAO pDAO;
	...
	public Map sjbusiness(int size,int page,String keywords) {
		/**
		 * 查询当前代理所代理的店铺
		 * 查询与当前代理同city且未被审核的（status==0）店铺
		 */
		//查出所有一级代理
		String hql3 = "from Proxy p where p.level=?";
		 List<Proxy> proxyList = hqldao.findByHQL(hql3, (short)1);
		//遍历一级代理，遍历一级代理的area
		StringBuffer city = new StringBuffer("(");
		if(proxyList.size()>0){
			System.out.println(proxyList.size());
			for (Proxy proxy2 : proxyList) {
				Proxy proxy = pDAO.findById(proxy2.getId());
		...
		String hqlsum = "select count(*) from Business where status=0 and area not in "+city;
		...
		if (page > count)
			page = count;		
		String hql = "from Business where status=0 and area not in "+city+ " order by status";		
